...
#---------Save the model----------------------------------------------------------------------------------------------------
model.save('C:\\Users\\chris\\Desktop\\Black_Rail_Audio_Detection\\models\\BLRA_model')

# Save model with the HDF5 Format
model.save('C:\\Users\\chris\\Desktop\\Black_Rail_Audio_Detection\\models\\BLRA_model.h5')
...
mp3 = 'C:\\Users\\chris\\Desktop\\Black_Rail_Audio_Detection\\data\\Audio_Files_To_Test_Model\\BLRA_Test_Clip2_Pos.mp3'
mp3
...
# Assuming each window is 1 second long
window_length_in_seconds = 1

# This list will store the start times of windows with positive predictions
positive_windows_start_times = []
...
# Randomly sample half of the frames
...
# Convert generator to a list and then create a Dataset
...
# Make predictions on the mp3 files
yhat = model.predict(audio_slices_sampled)

# Convert predictions to binary labels
yhat_binary = [1 if prediction > 0.95 else 0 for prediction in yhat]

# Determine the start times of positive windows
for i, prediction in enumerate(yhat_binary):
    if prediction == 1:
        start_time = i * window_length_in_seconds
        positive_windows_start_times.append(start_time)

print("Positive predictions were made at the following start times (in seconds):", positive_windows_start_times)


#-----------Make predicitons on the mp3 files in an entire folder----------------
...
results = {}
for file in 'C:\\Users\\chris\\Desktop\\Black_Rail_Audio_Detection\\data\\Audio_Files_To_Test_Model':
    FILEPATH = ('C:\\Users\\chris\\Desktop\\Black_Rail_Audio_Detection\\data\\Audio_Files_To_Test_Model', file)
...
with open('results.csv', 'w', newline='') as f:
    ...
    writer.writerow(['recording', 'BLRA calls'])
